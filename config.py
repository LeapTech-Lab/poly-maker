...
class BotConfig:
    ...
    price_spread_bps 是旧报价参数；当前 FOK 主模式不使用它。
    主模式使用 FOK 激进预测限价单，预测正确且流动性足够就立即全成交，否则整单取消。
    ...
    order_type: str = (_env("ORDER_TYPE", "FOK") or "FOK").upper()
    post_only: bool = _bool("POST_ONLY", "false")
    ...
    def validate(self) -> None:
        ...
        if self.order_type != "FOK":
            raise ValueError("ORDER_TYPE must be FOK for the current strategy")
        if self.post_only:
            raise ValueError("POST_ONLY must be false when ORDER_TYPE=FOK")
